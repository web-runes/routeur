...
Routeur uses pnpm workspaces, so you should **always run `pnpm install` from the top-level project directory**. Running `pnpm install` in the top-level project root will install dependencies for every package in the repo.
...
When making a pull request, be sure to add a changeset when something has changed with Routeur. Non-packages (`examples/*`) do not need changesets.
